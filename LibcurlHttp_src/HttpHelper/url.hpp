...
public:
    ...
    Url &add_query(const std::string &key, const std::string &val)
        {lazy_parse(); m_built=false; m_query.emplace_back(KeyVal(key,val)); return *this;}

	Url &add_query(const std::string &key, const char* szVal)
	{
		lazy_parse(); m_built = false; m_query.emplace_back(KeyVal(key, szVal)); return *this;
	}

	Url &add_query(const std::string &key, int ival)
	{
		char val[20];
		sprintf(val, "%d", ival);
		lazy_parse(); m_built = false; m_query.emplace_back(KeyVal(key, val)); return *this;
	...
	Url &add_query(const std::string &key, bool bval)
	{
		std::string val;
		if (bval) val = "true"; else val = "false";
		lazy_parse(); m_built = false; m_query.emplace_back(KeyVal(key, val)); return *this;
	}
