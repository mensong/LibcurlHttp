 ...
 */
void
RestClient::Connection::SetHeaders(RestClient::RequestHeaderFields headers) {
 ...
 *
 * @returns a RestClient::RequestHeaderFields map containing the custom headers
 */
RestClient::RequestHeaderFields
...
#else
	if (this->customUserAgent.length() > 0 && this->customUserAgent[0] != '\0') {
		return this->customUserAgent;
	}
	return std::string(std::string("restclient-cpp/") + RESTCLIENT_VERSION);
#endif
}
...
RestClient::Connection::performCurlRequest(const std::string& uri) {
  ...
  /** set http headers */
  for (RequestHeaderFields::const_iterator it = this->headerFields.begin();
  ...
  {
	curl_easy_setopt(this->curlHandle, CURLOPT_SSL_VERIFYPEER, 1L);//Gergul
